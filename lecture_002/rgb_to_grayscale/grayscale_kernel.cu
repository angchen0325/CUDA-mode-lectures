...
__global__
void rgb2grayscale_kernel(unsigned char* output, unsigned char* input, int width, int height) {
...
torch::Tensor rgb2grayscale(torch::Tensor image) {
    ...
    rgb2grayscale_kernel<<<number_of_blocks, threads_per_block, 0, torch::cuda::getCurrentCUDAStream()>>>(
